...
SUPPORTED_BOARDS=(
	...
	[e302]=e300 # ER-12 (EdgeRouter 12)
	[e303]=e300 # ER-12 (EdgeRouter 12P)
...
latest_release_for() {
	...
	curl -sSL -k https://api.github.com/repos/${WIREGUARD_REPO}/releases \
		| jq -r \
			--arg version "${board}" \
			'map(select(.prerelease == false)) | sort_by(.created_at) | reverse | .[0] | .tag_name as $tag_name
...
run_self_update() {
	...
	cat <<-EOS >"$tmpdir"/wireguard-script-self-update.sh
		...
		curl -sSL -k https://github.com/mafredri/vyatta-wireguard-installer/raw/master/wireguard.sh -o \$NEW_SCRIPT
